...
def test_evaluate_approves_a_normal_order():
    engine = RiskEngine(_risk_config(), 2000.0)
...
def test_kill_switch_triggers_on_drawdown_and_stays_triggered():
    engine = RiskEngine(_risk_config(max_portfolio_drawdown=0.10), 2000.0)
    engine.update_equity(2000.0, _ts())
...
def test_no_kill_switch_below_drawdown_threshold():
    engine = RiskEngine(_risk_config(max_portfolio_drawdown=0.10), 2000.0)
    engine.update_equity(2000.0, _ts())
...
def test_cooldown_activates_after_consecutive_losses_and_expires():
    engine = RiskEngine(_risk_config(cooldown_after_consecutive_losses=CooldownConfig(losses=2, cooldown_bars=3)), 2000.0)
    engine.update_equity(2000.0, _ts())
...
def test_a_winning_trade_resets_consecutive_loss_count():
    engine = RiskEngine(_risk_config(cooldown_after_consecutive_losses=CooldownConfig(losses=2, cooldown_bars=3)), 2000.0)
    engine.record_trade_close(-10.0)
...
def test_daily_trade_limit_blocks_further_orders_same_day():
    engine = RiskEngine(_risk_config(daily_trade_limit=2), 2000.0)
    engine.update_equity(2000.0, _ts())
...
def test_daily_trade_count_resets_on_a_new_calendar_day():
    engine = RiskEngine(_risk_config(daily_trade_limit=1), 2000.0)
...
def test_max_daily_loss_blocks_new_orders():
    engine = RiskEngine(_risk_config(max_daily_loss=0.02), 2000.0)
...
def test_max_weekly_loss_blocks_new_orders():
    engine = RiskEngine(_risk_config(max_weekly_loss=0.05), 2000.0)
...
def test_weekly_tracker_resets_on_a_new_iso_week():
    engine = RiskEngine(_risk_config(max_weekly_loss=0.05), 2000.0)
...
def test_max_open_positions_blocks_further_orders():
    engine = RiskEngine(_risk_config(max_open_positions=1), 2000.0)
    engine.update_equity(2000.0, _ts())
...
def test_insufficient_margin_blocks_an_oversized_order():
    engine = RiskEngine(_risk_config(max_leverage=10, max_margin_usage=0.5), 2000.0)
    engine.update_equity(2000.0, _ts())
...
def test_sufficient_margin_allows_a_reasonably_sized_order():
    engine = RiskEngine(_risk_config(max_leverage=30, max_margin_usage=0.5), 2000.0)
    engine.update_equity(2000.0, _ts())
